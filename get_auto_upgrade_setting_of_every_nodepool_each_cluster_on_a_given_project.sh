...
echo
gcloud container node-pools list --cluster $CLUSTER --region $REGION |  awk '{print $1}' | grep -v NAME | sort > ./tmp.nodepools
cat ./tmp.nodepools
echo
echo
while read n; do
  echo "$n"
  gcloud container node-pools describe $n --cluster=$CLUSTER --region=$REGION | grep -i autoupgrade
done <./tmp.nodepools
